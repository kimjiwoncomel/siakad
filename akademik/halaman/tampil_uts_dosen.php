...
while ($data_nilai = mysql_fetch_array($ambil_nilai)){
		...
		mysql_query("update uts set nilai_khs_bobot='$nilai[$i]', nilai_khs_huruf='$huruf' where id_krs_perawat='$id_krs_perawat[$i]'") ;
		echo mysql_error();
		}
}
  echo"<html><head><title></title><meta http-equiv='refresh'content='1;URL=file.php?file=hasil input nilai uts dosen&sinyal=$sinyal&identifikasi=$identifikasi&kategori=$kategori&token=$token&id_prodi=$id_prodi&id_smt=$id_smt&th_akademik=$th_akademik&id_kelas=$id_kelas&id_mk=$id_mk></head><body><center><strong><font color='#FF0000'>Data Diproses...</font></strong></center></body></html>";
  echo mysql_error();
exit();
}
...
for ($i=2; $i<=$baris; $i++)
{
 ...
 $query = "update uts set nilai_khs_bobot='$nilai_khs_bobot', nilai_khs_huruf='$nilai_khs_huruf' where id_krs_perawat='$id_krs_perawat'";
...
echo"<html><head><title></title><meta http-equiv='refresh'content='1;URL=file.php?file=hasil input nilai uts dosen&sinyal=$sinyal&identifikasi=$identifikasi&kategori=$kategori&token=$token&id_prodi=$id_prodi2&id_kurikulum=$id_kurikulum2&id_kelas=$id_kelas2&th_akademik=$th_akademik2&id_mk=$id_mk2&id_smt=$id_smt2'></head><body><center><strong><font color='#FF0000'>Data Diproses...</font></strong></center></body></html>";
echo mysql_error();
exit();
}
?>
...
<div align="center"><BR />
  <strong>[<? echo"<a href='./halaman/download_nilai2.php?sinyal=$sinyal&identifikasi=$identifikasi&kategori=$kategori&token=$token&id_prodi=$id_prodi&id_kurikulum=$id_kurikulum&id_mk=$id_mk&th_akademik=$th_akademik&id_smt=$id_smt&id_kelas=$id_kelas'>Download Tabel Upload Nilai $data_mk[matkul]</a>";?>]  </strong></div>
  <br />
...
<?
$cekquery= "SELECT *from uts where id_prodi='$id_prodi' and  id_kelas='$id_kelas' and id_smt='$id_smt' and th_akademik='$th_akademik' and id_mk='$id_mk'";
