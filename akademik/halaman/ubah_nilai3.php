...
$id_program2=$_POST['id_program2'];
$th_masuk2=$_POST['th_masuk2'];
 $sinyal = $_POST['sinyal'];
$kategori = $_POST['kategori'];
$id_prodi = $_POST['id_prodi'];
$id_kurikulum = $_POST['id_kurikulum'];
...
$sql="insert into lokal(id_mhs,nim,nama,program,jurusan,materi,nilai) 
values('$id_mhs','$nim','$nama','$program','$jurusan','$materi','$nilai')";
$simpan=mysql_query($sql);
//bila berhasil simpan kembali ke home
if ($simpan) {
echo"<html><head><title></title><meta http-equiv='refresh'content='0;URL='home_admin.php?id_prodi=$id_prodi&file=proses nilai lokal input&sinyal=$sinyal&identifikasi=$identifikasi&kategori=$kategori&token=$token&id_mhs=$id_mhs&th_masuk=$th_masuk&id_kelas=$id_kelas&id_program=$id_program&id_kurikulum=$id_kurikulum'></head><body><strong><font color='#0000FF'><center>Data diproses ...</center></font></strong></body></html>";
	} 
    else 
    { 
...
$sql2="select * from lokal where id='$id';";
...
$SQL = "UPDATE lokal SET 	materi='$materi',nim='$nim',nilai='$nilai',id_mhs='$id_mhs',jurusan='$jurusan',program='$program' where id='$id2'"; 
  	$hasil= mysql_query($SQL); 
	//jika berhasil kembali ke home
  	if($hasil){
    echo"<html><head><title></title><meta http-equiv='refresh'content='0;URL=home_admin.php?id_prodi=$id_prodi&file=proses nilai lokal input&sinyal=$sinyal&identifikasi=$identifikasi&kategori=$kategori&token=$token&id_kelas=$id_kelas&id_mhs=$id_mhs2&th_masuk=$th_masuk2&id_program=$id_program2&id_kurikulum=$id_kurikulum2'></head><body><strong><font color='#0000FF'><center>Data diproses ...</center></font></strong></body></html>";
	} 
    else 
    { 
	echo "<script type='text/javascript'>
...
$th_masuk=$_GET[th_masuk];
$id_prodi=$_GET['id_prodi'];
$SQL = "delete from lokal where id='$id'"; 
 if(mysql_query($SQL)) 
  { 
  echo"<html><head><title></title><meta http-equiv='refresh'content='0;URL=home_admin.php?id_prodi=$id_prodi&file=proses nilai lokal input&sinyal=$sinyal&identifikasi=$identifikasi&kategori=$kategori&token=$token&id_mhs=$id_mhs&th_masuk=$th_masuk&id_kelas=$id_kelas&id_program=$id_program&id_kurikulum=$id_kurikulum2'></head><body><strong><font color='#0000FF'><center>Data diproses ...</center></font></strong></body></html>";
	} else {
...
<? echo include "khs_krs.php";?>
        ...
        <h3><center><font color="#000">HALAMAN DATA NILAI UAP LOKAL</font></center></h3>
...
$x="select * from lokal where id_mhs='$data2[id_mhs]'";
//ambil query tampilkan
